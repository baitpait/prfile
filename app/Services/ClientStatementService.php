...
class ClientStatementService
{
     ...
     *
     * Balance = total invoiced − total paid − total adjustments (positive = client still owes).
     */
    public function forClient(Client $client, ?string $dateFrom = null, ?string $dateTo = null): array
    {
        ...
        $adjustmentsQuery = $client->balanceAdjustments()
            ->whereNull('deleted_at');
        ...
        foreach ($currencies as $currency) {
            ...
            $result[$currency] = [
                ...
                'balance' => $totalInvoiced - $totalPaid - $totalAdjusted,
                'timeline' => $timeline,
            ];
        }
    ...
    /**
     * Business Purpose: Export movements in chronological order plus period totals.
     */
    public function toCsvRows(array $statement): array
    {
        ...
        foreach ($statement as $currency => $section) {
            ...
            $rows[] = [$currency, '', 'الرصيد المستحق', number_format((float) $section['balance'], 2, '.', '')];
            $rows[] = ['', '', '', ''];
        }
