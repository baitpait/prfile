...
class MigrateLegacyData extends Command
{
    ...
    public function handle(): int
    {
        ...
        DB::transaction(function () use ($legacy) {
            ...
            $this->migrateIncomeEntries($legacy);
    ...
    private function migrateClients($legacy): void
    {
        ...
        foreach ($rows as $r) {
            if (DB::table('clients')->where('id', $r->id)->exists()) continue;
    ...
    private function migrateClientContacts($legacy): void
    {
        ...
        foreach ($rows as $r) {
            if (DB::table('client_contacts')->where('id', $r->id)->exists()) continue;
    ...
    private function migrateSuppliers($legacy): void
    {
        ...
        foreach ($rows as $r) {
            if (DB::table('suppliers')->where('id', $r->id)->exists()) continue;
    ...
    private function migrateSupplierContacts($legacy): void
    {
        ...
        foreach ($rows as $r) {
            if (DB::table('supplier_contacts')->where('id', $r->id)->exists()) continue;
    ...
    private function migrateInvoices($legacy): void
    {
        ...
        foreach ($rows as $r) {
            if (DB::table('invoices')->where('id', $r->id)->exists()) continue;
    ...
    private function migrateInvoiceLines($legacy): void
    {
        ...
        foreach ($rows as $r) {
            if (DB::table('invoice_lines')->where('id', $r->id)->exists()) continue;
        ...
        if ($chunk) DB::table('invoice_lines')->insert($chunk);
    ...
    private function migratePurchaseOrders($legacy): void
    {
        ...
        foreach ($rows as $r) {
            if (DB::table('purchase_orders')->where('id', $r->id)->exists()) continue;
    ...
    private function migratePurchaseOrderLines($legacy): void
    {
        ...
        foreach ($rows as $r) {
            if (DB::table('purchase_order_lines')->where('id', $r->id)->exists()) continue;
        ...
        if ($chunk) DB::table('purchase_order_lines')->insert($chunk);
        $this->line("  → {$inserted} بند");
    }
    ...
    private function migrateClientPayments($legacy): void
    {
        ...
        foreach ($rows as $r) {
            if (DB::table('client_payments')->where('id', $r->id)->exists()) continue;
    ...
    private function migrateSupplierPayments($legacy): void
    {
        ...
        foreach ($rows as $r) {
            if (DB::table('supplier_payments')->where('id', $r->id)->exists()) continue;
    ...
    private function migrateExpenses($legacy): void
    {
        ...
        foreach ($rows as $r) {
            if (DB::table('expenses')->where('id', $r->id)->exists()) continue;
    ...
    private function migrateIncomeEntries($legacy): void
    {
        $rows = $legacy->table('income_entries')->get();
        $this->info("إيرادات: {$rows->count()}");
        $inserted = 0;

        foreach ($rows as $r) {
            if (DB::table('income_entries')->where('id', $r->id)->exists()) continue;
    ...
    private function cleanNotes(?string $notes): ?string
    {
        ...
        if (str_contains($notes, 'CreditLimit') || str_contains($notes, '=0')) return null;
        $trimmed = trim($notes);
