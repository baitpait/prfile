...
class ClientAdjustmentsPeriodReport extends Component
{
    ...
    public function mount(): void
    {
        ...
        $this->currencyOptions = (new ClientAdjustmentsPeriodReportService)->currencyOptions();
    ...
    {
        return route('reports.client-adjustments.pdf', $this->periodQueryParams());
    }
    ...
    public function exportCsv(): StreamedResponse
    {
        ...
        $filename = 'تسويات-العملاء-'.now()->format('Y-m-d').'.csv';

        return response()->streamDownload(function () use ($rows, $filters): void {
            $handle = fopen('php://output', 'w');
            fprintf($handle, chr(0xEF).chr(0xBB).chr(0xBF));
            fputcsv($handle, ['التاريخ', 'العميل', 'النوع', 'السبب', 'العملة', 'المبلغ']);
            foreach ($rows as $r) {
                fputcsv($handle, [
                    $r['date']->format('Y-m-d'),
                    $r['client_name'],
    ...
    {
        return view('livewire.reports.client-adjustments-period-report', [
