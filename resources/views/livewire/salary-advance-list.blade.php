...
<div class="flex items-center justify-between mb-6">
    <div>
        <h1 class="text-xl font-bold text-[#3D3D3D]">سلف الموظفين</h1>
        <p class="text-sm text-gray-400 mt-0.5">{{ $rows->total() }} سجل</p>
    </div>
    @can('create', App\Models\SalaryAdvance::class)
    <a href="{{ route('salary-advances.create') }}" wire:navigate class="btn btn-primary" style="text-decoration:none;">
...
<form wire:submit.prevent="applyListFilters" class="bg-white border border-[#E0E0E0] rounded p-4 mb-6">
    <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-6 items-end">
        ...
        <div>
            ...
            <input type="date" wire:model="dateFrom" dir="ltr" class="border border-[#E0E0E0] rounded px-3 py-2 text-sm w-full">
        </div>
        <div>
            <label class="block text-xs font-medium text-[#3D3D3D] mb-1">إلى تاريخ</label>
            <input type="date" wire:model="dateTo" dir="ltr" class="border border-[#E0E0E0] rounded px-3 py-2 text-sm w-full">
        </div>
...
<div class="card overflow-hidden">
    <table class="data-table">
        ...
        <tbody>
            ...
            <tr>
                <td>
                    <div class="font-medium">{{ $adv->employee?->displayName() ?? '—' }}</div>
                </td>
                <td class="font-mono text-sm" dir="ltr">{{ $adv->paid_at?->format('Y-m-d') }}</td>
                <td class="font-mono font-semibold text-sm text-[#C9A227]" dir="ltr">{{ number_format((float)$adv->amount, 2) }} {{ $adv->currency_code }}</td>
                <td>{{ App\Models\SalaryAdvance::statusLabel($adv->status) }}</td>
                <td>
                    <div class="flex items-center gap-1 justify-end">
                        <a href="{{ route('salary-advances.show', $adv) }}" wire:navigate class="btn btn-ghost py-1 px-2 text-xs">عرض</a>
                        @can('update', $adv)
                        <a href="{{ route('salary-advances.edit', $adv) }}" wire:navigate class="btn btn-ghost py-1 px-2 text-xs text-blue-600">تعديل</a>
                        <button type="button" wire:click="confirmDelete({{ $adv->id }})" class="btn btn-ghost py-1 px-2 text-xs text-red-500">حذف</button>
                        @endcan
                    </div>
                </td>
            </tr>
            @empty
            <tr><td colspan="5" class="text-center py-16 text-gray-300">لا توجد سلف مسجّلة</td></tr>
            @endforelse
        </tbody>
    </table>
...
<div class="fixed inset-0 z-[60] flex items-center justify-center">
    ...
    <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 z-10 p-6">
        <h3 class="text-base font-bold text-center mb-4">حذف السلفة؟</h3>
