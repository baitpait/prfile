...
**مرجع الدفعة في الكشف:** `bank_reference` ثم `notes` ثم `#id` — عبر `HasStatementPaymentReference` على `ClientPayment` / `SupplierPayment`.

---

## 7ب) عرض كشف حساب المورد (محاذاة مع العميل)

### ملخص أعلى كل عملة

1. إجمالي أوامر الشراء  
2. إجمالي الدفعات  
3. **إجمالي التسويات**  
4. **المتبقي للمورد**  
5. نص: `المتبقي = أوامر الشراء − الدفعات − التسويات`

### جدول الحركات (ترتيب زمني)

| نوع الحركة | عمود المبلغ | تفاصيل |
|------------|-------------|--------|
| أمر شراء | `+` | **بنود أمر الشراء** (جدول فرعي: بند، تفاصيل، كمية، سعر، إجمالي) |
| دفعة | `−` | مرجع موحّد (انظر أعلاه) |
| تسوية | `−` (بنفسجي) | نوع + سبب |

- **العنوان:** «كشف حساب» + اسم المورد (مثل العميل).
- **PDF:** `resources/views/pdf/supplier-statement.blade.php`
- **CSV:** `SupplierStatementService::toCsvRows()`
